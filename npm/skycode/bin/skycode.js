...
const pkg = `@skycode/cli-${process.platform}-${process.arch}`;
const exe = `skycode${process.platform === "win32" ? ".exe" : ""}`;
...
try {
  ...
  console.error(
    `skycode: no prebuilt binary for ${process.platform}-${process.arch}.\n` +
