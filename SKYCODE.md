...
agent. It is the Skycode analog of Claude Code's CLAUDE.md.
...
- Standing instructions are hierarchical: committed/shared `SKYCODE.md`,
  ...
  (`SKYCODE_STATE_HOME`, otherwise `SKYCODE_HOME`, otherwise `~/.skycode` on
  macOS/Linux or `%APPDATA%\skycode` on Windows). All distinct supported files
