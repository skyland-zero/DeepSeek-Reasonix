...
func (c *Controller) EnableInteractiveApproval() {
	...
	// [MODIFIED] To make plan mode non-blocking, we do not register the TUI
	// approver; the coordinator then takes its unregistered default path
	// (persist the plan, surface the ready notice, run nothing).
	// if setter, ok := c.runner.(interface {
